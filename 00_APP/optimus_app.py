...
@media(max-width:980px){.app,.app.admin-open{display:block;height:100vh}.sidebar{position:fixed;z-index:30;top:0;bottom:0;left:0;width:min(340px,88vw);transform:translateX(-105%);transition:transform .2s ease;box-shadow:12px 0 32px rgba(0,0,0,.28)}.app.sidebar-open .sidebar{transform:translateX(0)}.sidebar-scrim{position:fixed;z-index:29;inset:0;background:rgba(0,0,0,.36)}.app.sidebar-open .sidebar-scrim{display:block}.app #divIzq,.app #divDer,.app .admin{display:none}.main{width:100%;height:100vh}.composer,.app.admin-open .composer{left:0;right:0;padding:22px 14px 14px}.top{height:auto;min-height:62px;padding:9px 12px;align-items:flex-start;gap:10px}.top>div:first-child{min-width:0;padding-top:3px}.top h1{font-size:14px;line-height:1.2}.top .actions{flex:1;justify-content:flex-end;flex-wrap:wrap;gap:6px}.top .pill{padding:7px 9px;font-size:12px}.sidebar-toggle{display:inline-flex;align-items:center;gap:5px}.chat{padding:20px 14px 190px}.thread{max-width:760px}.welcome{margin:4vh auto 24px}.welcome h2{font-size:24px}.cards{grid-template-columns:1fr}.card{padding:12px}.report-editor{min-height:330px;padding:18px}.status{white-space:normal}.box{padding:11px}.box textarea{min-height:82px}}
...
function toggleSidebar(){const shell=$('appShell');const open=!shell.classList.contains('sidebar-open');shell.classList.toggle('sidebar-open',open);const button=$('sidebarToggle');if(button)button.setAttribute('aria-expanded',String(open))}
const MODELOS_BASE_POR_PROVEEDOR={openai:['gpt-4.1-mini','gpt-4.1','gpt-4o-mini'],anthropic:['claude-sonnet-4-5','claude-opus-4-1'],deepseek:['deepseek-chat','deepseek-reasoner'],llama_cpp:[],mock:['mock-radiology']};
function claveModelosProveedor(proveedor){return 'fab_models_'+proveedor}
function modelosBloqueados(proveedor){try{return JSON.parse(localStorage.getItem(claveModelosProveedor(proveedor)+'_blocked')||'[]')}catch(e){return []}}
function modelosDisponiblesProveedor(){const proveedor=$('provider')?.value||'openai';let detectados=[];try{detectados=JSON.parse(localStorage.getItem(claveModelosProveedor(proveedor))||'[]')}catch(e){}const modelos=detectados.length?detectados:(MODELOS_BASE_POR_PROVEEDOR[proveedor]||[]);const bloqueados=new Set(modelosBloqueados(proveedor));return modelos.filter(modelo=>!bloqueados.has(modelo))}
function guardarModelosProveedor(proveedor,modelos){localStorage.setItem(claveModelosProveedor(proveedor),JSON.stringify([...new Set(modelos)].filter(Boolean)))}
...
const providerChangedBase=providerChanged;
providerChanged=function(){providerChangedBase();asegurarSelectorRapidoModelo()}
...
async function detectarModelos(){$('estado').textContent='Consultando modelos…';const proveedor=$('provider').value;const r=await fetch('/modelos',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(cfg())});const d=await r.json();if(d.error){$('estado').textContent=mensajeErrorProveedor(proveedor,d.error);return}guardarModelosProveedor(proveedor,d.models||[]);const dl=$('model-presets');dl.innerHTML=(d.models||[]).map(m=>`<option value="${m}">`).join('');if(d.preferido&&modelosDisponiblesProveedor().includes(d.preferido))$('model').value=d.preferido;sincronizarSelectorRapidoModelo();$('estado').textContent=modelosDisponiblesProveedor().length+' modelos disponibles para '+proveedor}
...
if(modelInput){modelInput.addEventListener('input',sincronizarSelectorRapidoModelo);modelInput.addEventListener('change',()=>{saveConfig();sincronizarSelectorRapidoModelo()})}
asegurarSelectorRapidoModelo();
