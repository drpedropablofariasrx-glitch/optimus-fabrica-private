...
<aside class="admin"><div class="admin-head"><b>Chat de sistema</b><span>Cambia formato, reglas o el prompt (con copia de seguridad)</span></div><div class="admin-log" id="adminLog"><div class="admin-msg bot">Pídeme cambios de formato o reglas: “quiero que el informe no incluya interpretación global”, “agrega un bloque de análisis global”, “crea una regla para no recomendar cardiología por calcificaciones coronarias leves”.</div><div class="admin-msg warn">Antes de aplicar verás el diff exacto (qué se quita y qué se añade). Cada cambio guarda copia de seguridad y puedes deshacerlo.</div></div><div class="admin-compose"><textarea id="adminText" placeholder="Escribe una instrucción para cambiar el formato o las reglas…"></textarea><div class="admin-actions"><button class="primary" onclick="adminChat()">Enviar</button><button onclick="guardarBorradorPrompt()">Guardar borrador</button><button onclick="aplicarPrompt()">Aplicar override</button><button onclick="restaurarPromptBase()">Restaurar base</button><button onclick="verPrompt()">Ver prompt</button><button onclick="verCandidatas()">Reglas aprendidas</button></div><div class="status" id="adminStatus"></div></div></aside>
</div>
<script>
...
function hayContenidoPendiente(){const inf=$('informe');return !!(($('caso')&&$('caso').value.trim())||currentCaseInput||(inf&&inf.innerText.trim()))}
async function regionChanged(){const sel=$('region');const nueva=sel.value;if(nueva===currentRegion)return;if(hayContenidoPendiente()&&!confirm('Cambiar de región limpiará el caso actual no guardado. ¿Continuar?')){sel.value=currentRegion;return}const r=await fetch('/region',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({region:nueva})});const d=await r.json();if(!r.ok||!d.ok){alert(d.error||'No se pudo cambiar de región');sel.value=currentRegion;return}currentRegion=d.region;$('regionTitle').textContent=regionLabel(currentRegion);nuevoCaso();renderThoraxControls();cargarCasos();adminAdd('bot','Región activa: '+(d.region_name||currentRegion)+' · prompt '+(d.prompt_version||''))}
...
function setReport(txt){marcarNoValidado();msg('assistant','Informe generado',`<div id="informe" class="report" contenteditable="true"></div><div class="correction" id="correctionCard"><label class="field">Qué corregiste y por qué</label><textarea id="correccion" placeholder="Ej.: cambié ectasia por ureteropielocaliectasia porque es el término preciso."></textarea></div><div class="row" style="margin-top:10px;justify-content:flex-start"><button class="pill primary" onclick="validarGold()">Validar como Gold Standard</button><span class="status" id="goldStatus">No validado como Gold Standard</span></div>`);$('informe').textContent=txt;resaltarEncabezados($('informe'));$('informe').addEventListener('input',()=>{$('correctionCard').style.display=($('informe').innerText.trim()!==informeIA.trim())?'block':'none';if(caseStatus!=='validated')caseStatus='corrected';marcarNoValidado()});$('informe').addEventListener('blur',()=>{const pos=window.getSelection().rangeCount;resaltarEncabezados($('informe'))})}
...
async function copiarInforme(){const inf=$('informe');if(!inf){alert('Genera un informe primero');return}
  ...
  const limpio=inf.innerText.replace(/\u00A0/g,' ');
...
async function cargarRegiones(){const r=await fetch('/regiones');const d=await r.json();currentRegion=d.current_region||'abdomen';const sel=$('region');if(sel){sel.innerHTML=d.regions.filter(x=>x.enabled).map(x=>`<option value="${x.region_id}">${x.region_name}</option>`).join('');sel.value=currentRegion}$('regionTitle').textContent=regionLabel(currentRegion);renderThoraxControls()}
