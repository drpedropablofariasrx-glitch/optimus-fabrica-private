...
function sincronizarSelectorRapidoModelo(){const input=$('model'),quick=$('quickModel'),presets=$('model-presets');if(!input||!quick||!presets)return;const modelos=[...new Set([...presets.options].map(option=>option.value).filter(Boolean).concat(input.value.trim()?[input.value.trim()]:[]))];quick.replaceChildren(...modelos.map(modelo=>new Option(modelo,modelo)));quick.value=input.value.trim()||modelos[0]||''}
...
const detectarModelosBase=detectarModelos;
detectarModelos=async function(){await detectarModelosBase();asegurarSelectorRapidoModelo()}
